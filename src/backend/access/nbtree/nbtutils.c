...
_bt_killitems(IndexScanDesc scan, BTScanState state)
{
	...
	/* Always invalidate state->killedItems[] before leaving so->currPos */
	state->numKilled = 0;

	/*
	 * We need to iterate through state->killedItems[] in leaf page order; the
	 * loop below expects this (when marking posting list tuples, at least).
	 * state->killedItems[] is now in whatever order the scan returned items in.
	 * Scrollable cursor scans might have even saved the same item/TID twice.
	 *
	 * Sort and unique-ify state->killedItems[] to deal with all this.
	 */
	if (numKilled > 1)
	{
		qsort(state->killedItems, numKilled, sizeof(int), _bt_compare_int);
		numKilled = qunique(state->killedItems, numKilled, sizeof(int),
							_bt_compare_int);
	}
	...
	for (int i = 0; i < numKilled; i++)
	{
		...
		Assert(i == 0 ||
			   offnum >= pos->items[state->killedItems[i - 1]].indexOffset);
...
int
_bt_init_knn_start_keys(IndexScanDesc scan, ScanKey *startKeys, ScanKey bufKey)
{
