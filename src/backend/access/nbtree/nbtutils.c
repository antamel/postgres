...
_bt_preprocess_array_keys(IndexScanDesc scan)
{
	...
	for (int i = 0; i < numberOfKeys; i++)
	{
		...
		orderByDistance = scan->numberOfOrderBys > 0 &&
		...
		sort_array_cmp = _bt_setup_array_cmp(scan, cur, elemtype, &so->orderProcs[i], &cxt);
		...
		else
		{
			...
			*/
			...
			if (orderByDistance && origarrayatt == cur->sk_attno && origreverse != cxt.reverse)
			{
				/*
		...
		if (origarrayatt == cur->sk_attno)
		{
			...
			if (orderByDistance && need_reverse)
			{
				need_reverse = false;
				 ...
				 Datum *elems_next, int nelems_next)
{
	...
	Datum 		*sort_result;
	bool orderByDistance = scan->numberOfOrderBys > 0 &&
						   scan->orderByData[0].sk_attno == skey->sk_attno;
...
static int
_bt_compare_array_element_with_dist_arg(const void *a, void *arg)
{
	BTSortArrayContext *cxt = (BTSortArrayContext *) arg;
...
_bt_force_advance_array_keys(IndexScanDesc scan, BTReadPageState *pstate)
{
	...
	so->needPrimScan = _bt_advance_array_keys_increment(scan, dir);
