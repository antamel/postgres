...
{
	FmgrInfo   *sortproc;		/* ORDER proc for sorting SAOP array itself or
								 * by the results of distproc */
	FmgrInfo   *distproc;		/* to get distance from array elem to ordering
								 * key */
	FmgrInfo   *cmpproc;		/* ORDER proc to compare array elems with each
								 * other */
	FmgrInfo   *cmpdistargproc; /* ORDER proc to compare array elem with
								 * ordering key arg */
...
_bt_preprocess_keys(IndexScanDesc scan)
{
	...
	if (numberOfKeys == 1)
	{
		...
		/* We can emit scan key and make it required for first index col */
		_bt_emit_scan_key(scan, &inkeys[0], 0);
	...
	for (int i = 0;; i++)
	{
		...
		if (i == numberOfKeys || inkey->sk_attno != attno)
		{
			...
			for (j = BTMaxSearchStrategyNumber; --j >= 0;)
			{
				if (xform[j].inkey)
				{
					_bt_emit_scan_key(scan, xform[j].inkey, priorNumberOfEqualCols);

					if (arrayKeyData)
						keyDataMap[so->numberOfKeys - 1] = xform[j].inkeyi;
				}
			}
		...
		else
		{
			...
			else
			{
				 ...
				 */
				_bt_emit_scan_key(scan, xform[j].inkey, numberOfEqualCols);
					 ...
					 FmgrInfo *orderproc, BTArrayKeyInfo *array, bool *qual_ok)
{
	 ...
	 * nor ascending nor descending. So use in-place removing of the
	 * non-matching elements instead of binsearch.
	 */
...
_bt_preprocess_array_keys(IndexScanDesc scan, int *new_numberOfKeys)
{
	...
	for (int input_ikey = 0; input_ikey < scan->numberOfKeys; input_ikey++)
	{
		...
		bool		need_restore_direction = false;
		BTOrderArgLocation order_arg_location = UNKNOWN_ORDER_ARG_LOCATION;
		...
		*cur = *inkey;
		...
		sort_array_cmp = _bt_setup_array_cmp(scan, cur, elemtype,
											 &so->orderProcs[numArrayKeyData], &cxt);
		...
		else
		{
			...
			{
				/*
				 * In case of distance ordering determine what case are we dealing
				 * with and setup the scan direction and the array sort order
				 * accordingly.
				 *
				 * There are three different possible dispositions of the
				 * ordering key argument relative to the array elements.
				 */
				...
				if (0 <= _bt_compare_array_element_with_dist_arg(&min_elem,
																 &cxt))
				{
					/*
					 * case A: Ordering key argument is less than all values
					 * in array
					 */
				...
				else
				{
					...
					if (0 >= _bt_compare_array_element_with_dist_arg(&max_elem,
																	 &cxt))
					{
						/*
						 * case B: Ordering key argument is greater than all
						 * values in array
						 */
					...
					{
						/*
						 * case C: Ordering key argument is within the array
						 * bounds
						 */
			...
			if (cxt.orderbydistance && origarrayatt == cur->sk_attno && origreverse != cxt.reverse)
			{
				 ...
				 */
				need_restore_direction = true;
				cxt.reverse = origreverse;
			}

			/*
			 * Sort the non-null array elements and eliminate any duplicates.
			 * Unless it's case C of distance ordering we must sort in the
			 * same ordering used by the index column, so that the arrays can
			 * be advanced in lockstep with the scan's progress through the
			 * index's key space. In case C of distance ordering. Elements are
			 * sorted simply by distance from the order key argument.
			 *
			 * The array elements are sorted and deduplicated in-place. If
			 * reverse is true, we sort in descending order.
			 */
			...
			else
			{
				size_t		i,
							j,
							eqdind, /* first index of the equidistant
									 * subsequence */
							eqdlen; /* length of the equidistant subsequence */
				bool		eqdstart = false;

				/*
				 * At the start of each iteration: i - element to be consider;
				...
				while (i < num_nonnulls)
				{
					if (eqdstart)
					{
							...
							_bt_compare_array_elements_by_distance(elem_values + i, elem_values + j, &cxt) != 0)
						{
							...
							/*
							 * In a equidistance subsequence of length 2
							 * either only the first elem is needed or both.
							 * No sence to sort it.
							 */
							if (i - eqdind > 2)
		...
		if (origarrayatt == cur->sk_attno)
		{
								 ...
								 elem_values, num_elems))
			{
				...
				if (cxt.orderbydistance &&
					...
					/*
					 * In case C the just merged array may transforms into A
					 * or B. Need to recheck that.
					 */
				{
					...
					{
						/*
						 * case A: Ordering key argument is less than all
						 * values in array
						 */
					...
					else
					{
						...
						{
							/*
							 * case B: Ordering key argument is greater than
							 * all values in array
							 */
			 ...
			 * scan keys/arrays.
			 */
			if (cxt.orderbydistance && need_restore_direction)
			{
				need_restore_direction = false;
 ...
 * proc for the scan, and a cxt->sortprocp pointing to its own separate space.
 ...
 * Otherwise, cxt->sortprocp will continue to point to caller's own space.  Either
 * way, cxt->sortprocp will point to a same-type ORDER proc (since that's the only
					...
					FmgrInfo *orderproc, BTSortArrayContext *cxt)
{
	...
	{
		/*
		 * Look up the appropriate cross-type comparison function in the
		 * opfamily.
		 *
		 * Use the opclass input type as the left hand arg type, and the array
		 * element type as the right hand arg type (since binary searches use
		 * an index tuple's attribute value to search for a matching array
		 * element).
		 *
		 * Note: it's possible that this would fail, if the opfamily is
		 * incomplete, but only in cases where it's quite likely that
		 * _bt_first would fail in just the same way (had we not failed before
		 * it could).
		 */
	...
	if (cxt)
	{
		...
		{
			/* Set same-type ORDER procs for caller */
			fmgr_info_cxt(orderproc->fn_oid, cxt->sortproc, so->arrayContext);
		}
		else
		{
			/*
			 * Look up the appropriate same-type comparison function in the
			 * opfamily.
			 *
			 * Note: it's possible that this would fail, if the opfamily is
			 * incomplete, but it seems quite unlikely that an opfamily would
			 * omit non-cross-type comparison procs for any datatype that it
			 * supports at all.
			 */
			cmp_proc = get_opfamily_proc(rel->rd_opfamily[skey->sk_attno - 1],
		...
		/*
		 * The current sortproc is for compare the array elems with each other
		 * but now we may compare by the distance to the ordering key
		 * argument. Before finding the desired by distance sortproc save
		 * current sortproc also as cmpproc.
		 */
		...
		{
			/*
			 * Need to sort array by distance to ordering key argument. So
			 * init procedures for distance calculation and comparison.
			 */
			ScanKey		distkey = &scan->orderByData[0];
			Oid			distargtype = distkey->sk_subtype;
			Oid			distopr;
			RegProcedure distproc;
			Oid			opfamily = rel->rd_opfamily[skey->sk_attno - 1];
			Oid			distanceType;

			if (!OidIsValid(distargtype))
				distargtype = rel->rd_opcintype[skey->sk_attno - 1];

			Assert(distkey->sk_strategy == BTNearestStrategyNumber);
			...
			if (!OidIsValid(distopr))
				elog(ERROR, "missing operator (%u,%u) for strategy %d in opfamily %u",
					 elemtype, distargtype, distkey->sk_strategy, opfamily);
			...
			if (distargtype == elemtype)
			{
				/*
				 * We can use same-type ORDER proc for compare array elems
				 * with ordering key argument
				 */
				cxt->cmpdistargproc = cxt->cmpproc = orderproc;
			}
			else
			{
				cmp_proc = get_opfamily_proc(rel->rd_opfamily[skey->sk_attno - 1],
											 elemtype, distargtype, BTORDER_PROC);
				if (!RegProcedureIsValid(cmp_proc))
					elog(ERROR, "missing support function %d(%u,%u) for attribute %d of index \"%s\"",
						 BTORDER_PROC, elemtype, distargtype,
						 skey->sk_attno, RelationGetRelationName(rel));
			...
			/*
			 * Look up the appropriate function in the
			 * opfamily for the distproc return values comparison.
			 */
			distanceType = get_func_rettype(distkey->sk_func.fn_oid);

			cmp_proc = get_opfamily_proc(rel->rd_opfamily[skey->sk_attno - 1],
											distanceType, distanceType, BTORDER_PROC);
			if (!RegProcedureIsValid(cmp_proc))
				elog(ERROR, "missing support function %d(%u,%u) for attribute %d of index \"%s\"",
						BTORDER_PROC, distanceType, distanceType,
						skey->sk_attno, RelationGetRelationName(rel));
				 ...
				 Datum *elems_next, int nelems_next)
{
	...
	if (next_cxt->orderbydistance)
	{
		...
		{
			/*
			 * If the elements of the arrays to be merged are of different
			 * types we need two different functions for distance
			 * calculations.
			 */
			ocxt = &orig_cxt;
		...
		for (int i = 0, j = 0; i < nelems_orig_start && j < nelems_next;)
		{
			...
			if (res == 0)
			{
				...
				eqdiststart = i;	/* start index of a subsequence of
									 * elements with the same distance */
				oelem = ++i + *elems_orig;
